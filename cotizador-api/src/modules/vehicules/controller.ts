...
import { createBrandSchema, createLineSchema, getVehiclesQuerySchema, updateBrandSchema, updateLineSchema, updateVehicleVersionSchema } from "./schema";
import { AppError } from "../../core/errors/appError";

const ALLOWED_SORT_FIELDS = ["descrip", "idlinea", "idversion", "codigo"];

export const getBrands = async (req: Request, res: Response) => {
  if (!req.user) throw new AppError("Usuario no autenticado", 403);

  const { page, limit, sortBy, sortOrder } = getPaginationParams(req.query, ALLOWED_SORT_FIELDS, "descrip");
...
export const updateBrand = async (req: Request, res: Response) => {
  ...
  if (Number.isNaN(idmarca)) throw new AppError("ID de marca invalido", 400);
...
export const updateLine = async (req: Request, res: Response) => {
  ...
  if (Number.isNaN(idlinea)) throw new AppError("ID de linea invalido", 400);
...
export const getVersionById = async (req: Request, res: Response) => {
  ...
  if (Number.isNaN(idversion)) throw new AppError("ID de version invalido", 400);
